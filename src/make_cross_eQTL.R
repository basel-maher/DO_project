...
load("./results/Rdata/cross_basic_cleaned.Rdata")#load cross object for QTL mapping
...
peer_factors = read.delim("./results/flat/factors_vst_qnorm.txt",stringsAsFactors = FALSE, row.names = rownames(dds.vst), sep=" ", header = FALSE)
...
#save
write.csv(do_covar_eqtl, "./results/flat/do_covar_eqtl.csv", quote = FALSE,row.names = FALSE)
...
chr <- c(1:19, "X")
write_control_file("./control_file_eqtl.json",
                   crosstype="do",
                   ...
                   covar_file="../flat/do_covar_eqtl.csv",
                   sex_covar="sex",
...
#read the cross file
cross_eqtl = read_cross2("./control_file_eqtl.json")
...
save(cross_eqtl, file = "../Rdata/cross_eqtl.Rdata")
