...
#kda_analyses#
all = read.csv("./results/flat/key_driver_analysis_sexcombined_sft4_TEST.csv", stringsAsFactors = F)
all_f = read.csv("./results/flat/key_driver_analysis_FEMALES_sft4_TEST.csv", stringsAsFactors = F)
all_m = read.csv("./results/flat/key_driver_analysis_MALES_sft5_TEST.csv", stringsAsFactors = F)
...
length(which(tolower(genes) %in% tolower(superset_in_networks)))
length(which(tolower(genes) %in% tolower(superset_in_networks)==FALSE))
...
bone_df = x[which(tolower(x$gene) %in% tolower(superset_in_networks)),]
not_bone_df = x[which(tolower(x$gene) %in% tolower(superset_in_networks) == FALSE),]
...
#we use this because the BAN was defined as being conencted to a bone gene with num_neib (3-step). Connected in this context is 3-step neighbors
#doesnt change between superset and superset_in_networks
